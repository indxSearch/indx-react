...
All notable changes to `@indxsearch/react` will be documented in this file.
...
### Changed
- **BREAKING**: Package renamed from `@indxsearch/intrface` to `@indxsearch/react`
...
### Technical Details
This release aligns `@indxsearch/react` with the official IndxCloudApi v1.0.2 Swagger specification. The breaking changes fix discrepancies between the package and the actual API that could cause runtime errors or unexpected behavior.
...
```bash
npm install @indxsearch/react@latest
```

Or with a specific version:

```bash
npm install @indxsearch/react@2.1.0
```
